...
def extract_song_data(url):
    ...
    title = soup.select_one('h1[class="h3 d-inline-block"]').text.strip()
    lyrics = soup.select_one('#lyric').text.strip()
    category = soup.select_one('.ibar a[href*="category"]').text.strip().lower()
    composers = ','.join([a.text.strip() for a in soup.select('a[href*="composer"]')])
    singers = ','.join([a.text.strip() for a in soup.select('a[href*="singer"]')])
...
'''
Save a bookmarklet to your browser to save a song from hopamviet.vn:
javascript:(function() { fetch('http://localhost:5000/save_song', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: window.location.href })}).then(response => response.json()).then(data => alert(data.message)); })();
'''
